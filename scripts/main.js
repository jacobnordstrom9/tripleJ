function getInputValue(){
    let movie = document.getElementById("theMovie").value;
    console.log("name of movie", movie)
...
.then(data =>{
    ...
    for (var i = 0; i <  20; i++){
        ...
        // fetch(`https://api.watchmode.com/v1/search/?apiKey=${jacobWatchModeApi}&search_field=name&search_value=${data.results[i].title}`)
        //     .then(result => result.json())
        //     .then(data2 =>{
        //         console.log("second data",data2)
        //     var movieTitle = data2.title_results[0].id;
        //     // fetch(`https://api.watchmode.com/v1/title/${movieTitle}/sources/?apiKey=${jacobWatchModeApi}`)
        //     // .then(result => result.json())
        //     // .then(data3 =>{
